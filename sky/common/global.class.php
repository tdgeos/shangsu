...
class Common_Global_Class
{
	...
	{
		global $config, $get, $post, $viewObject;
		...
		$this->browserTitle = $this->config['browserTitle'];
		
		$className = $this->loadClass( $this->config['multipleAdapter'] );
		$_ENV['multipleObj'] = new $className( $this );
		
		$this->view->skyUrl = $this->config['flash']['skyUrl'];
	...
	function db()
	{
		...
		{
			$this->_db = $this->dbObj();
			$conn = $this->_db->init( $this->config['db']['host'], $this->config['db']['user'], $this->config['db']['password'], $this->config['db']['dbName'], $this->config['db']['charset'] );
			
			if( $conn == false )
			{
				
				$this->message( '数据库连接失败', '请联系管理员', 'failed' );
			}
	...
	function loadClass( $className )
	{
		...
		$classDir = SKY_ROOT."/{$classDir}{$classInfo[$size]}.class.php";
	...
	{
		include_once( SKY_ROOT.'/view/'.$template.'.php' ) ;
	}
	...
	function checkLogin()
	{
		return $_ENV['multipleObj']->userinfo();
	}
	
	function dateFormat( $date, $format = 'full', $timeZone = true )
	{
		if( $timeZone == true )
		$date = $date+$this->config['timezone']*3600;
